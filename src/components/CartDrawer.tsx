...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  const handleApplyPromo = (e: React.FormEvent) => {
    ...
    if (validation.isValid) {
      ...
      setPromoError(`Cupón no válido. Prueba con "${BUSINESS_RULES.DEFAULT_PROMO_CODE}"`);
    }
  };
          ...
          <div className="bg-slate-50 px-6 py-3 border-b border-slate-200/70">
            <div className="flex items-center justify-between text-xs mb-1.5">
              <div className="flex items-center gap-1.5 font-medium">
                ...
                {isFreeShipping ? (
                  <span className="text-emerald-700 font-semibold">¡Envío gratuito conseguido!</span>
                ) : (
                  <span className="text-slate-700">
                    Añade <strong>{remainingForFreeShipping.toFixed(2)}€</strong> más para envío gratis
                  </span>
                )}
              </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 divide-y divide-slate-100">
            {cartItems.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center py-12">
                ...
                <h3 className="text-sm font-semibold text-slate-800">Tu bolsa está vacía</h3>
                <p className="text-xs text-slate-500 mt-1 max-w-xs">
                  Explora nuestro catálogo minimalista y descubre prendas con cortes atemporales y tejidos de calidad.
                </p>
                <button
          ...
          {cartItems.length > 0 && (
            <div className="p-6 border-t border-slate-200 bg-slate-50/70 space-y-3">
              ...
              {appliedPromo && (
                <div className="flex items-center justify-between text-xs bg-emerald-50 text-emerald-800 px-3 py-1.5 rounded-lg border border-emerald-200">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>Descuento <strong>{appliedPromo.code}</strong> (-{appliedPromo.percent}%)</span>
                  </div>
                  <button
              ...
              <div className="space-y-1.5 pt-2 text-xs text-slate-600">
                ...
                <div className="flex justify-between">
                  <span>Envío</span>
                  <span>{shippingCost === 0 ? 'Gratuito' : `${shippingCost.toFixed(2)}€`}</span>
                </div>
